...
use Drupal\thron\Exception\BrandNotSetException;
...
use Drupal\thron\Exception\UploadPermissionException;
use Drupal\thron\Exception\UploadFailedException;
use Drupal\thron\Plugin\media\Source\THRON;
 ...
 * @EntityBrowserWidget(
 *   id = "thron_upload",
 *   label = @Translation("THRON Upload"),
 *   description = @Translation("Uploads files to THRON and creates wrapping media entities.")
 * )
 */
class THRONUpload extends THRONWidgetBase {
   ...
   */
  public function __construct(array $configuration, $plugin_id, $plugin_definition, EventDispatcherInterface $event_dispatcher, EntityTypeManagerInterface $entity_type_manager, WidgetValidationManager $validation_manager, ConfigFactoryInterface $config_factory, LoggerChannelFactoryInterface $logger_factory, SessionInterface $session, RequestStack $request_stack, THRONApiInterface $thron_api) {
    parent::__construct($configuration, $plugin_id, $plugin_definition, $event_dispatcher, $entity_type_manager, $validation_manager, $logger_factory, $request_stack, $config_factory, $thron_api);
  ...
  public function defaultConfiguration() {
    return [
      'submit_text' => $this->t('Upload entities'),
      'brand' => NULL,
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    $form['todo'] = [
      '#markup' => '<h4>TODO:</h4><p>This widget is not yet developed</p>',
    ];

    /*
    $brand_options = [];
    try {
      foreach ($this->THRONApi->getBrands() as $brand) {
    ...
    $metaproperties = $this->THRONApi->getMetaproperties();
    $metaproperty_options = [];
    ...
    $form['metaproperties'] = [
      ...
      '#ajax' => [
        'callback' => 'Drupal\thron\Plugin\EntityBrowser\Widget\thronUpload::ajaxMetaproperties',
...
//    unset($this->configuration['metaproperties']);
  ...
  public function getForm(array &$original_form, FormStateInterface $form_state, array $additional_widget_parameters) {
    ...
    $has_upload_permissions = $this->THRONApi->hasPermission('thron upload media');
    if (!$has_upload_permissions) {
      $form['actions']['submit']['#access'] = FALSE;
      $form['warning'] = [
        '#markup' => '<h4>' . $this->t('Access denied') . '</h4>',
      ];
      $form['message'] = [
        '#markup' => '<p>' . $this->t("Unable to upload files to THRON. Make sure your user account has role with enough permissions.") . '</p>',
      ];
    }
    else {
      $form['warning'] = [
        '#markup' => '<h4>Work in progress</h4>',
      ];
      $form['todo'] = [
        '#markup' => '<p>This tab pane is not yet developed</p>',
      ];
      $form['actions']['submit']['#access'] = FALSE;
    }
  ...
  public function submit(array &$element, array &$form, FormStateInterface $form_state) {
    if (!empty($form_state->getTriggeringElement()['#THRON_upload_submit'])) {
      /** @var \Drupal\media\MediaTypeInterface $type */
      $type = $this->entityTypeManager->getStorage('media_type')
        ->load($this->configuration['media_type']);

      if ($type && ($type->getSource()) instanceof THRON) {
        $form_state->setRebuild();
        $batch = [
          'title' => $this->t('Uploading assets to THRON'),
        ...
        $this->session->set('THRON_upload_batch_uuid', $form_state->get(['entity_browser', 'instance_uuid']));
        batch_set($batch);
        ...
        if (!$type) {
          ...
          (new BundleNotTHRONException($type->label()))->logException()->displayMessage();
        }
  ...
  /**
   * Upload batch operation callback which uploads assets to THRON.
   */
  public static function batchUploadFiles($file, $brand, $description, $tags, $metaproperty_options, &$context) {
    try {
      ...
      $context['message'] = t('Uploaded @file to THRON.', ['@file' => $file['filename']]);
    }
    catch (\Exception $e) {
      ...
      $context['finished'] = 0;
      $context['message'] = t('Uploading @file to THRON.', ['@file' => $file['filename']]);
  ...
  public static function batchCreateEntities($file, $source_field, $bundle, &$context) {
    try {
      if (\Drupal::service('session')->get('upload_permissions', FALSE) != 'MEDIAUPLOADFORAPPROVAL') {
        ...
        drupal_set_message(t("Your file was uploaded to THRON but needs to be approved before you can use it. Please go to your THRON waiting room and review the uploaded assets."), 'warning');
      }
      $context['finished'] = 1;
    }
    catch (\Exception $e) {
      ...
      if ($context['sandbox']['fails'] >= static::FAIL_LIMIT) {
        (new UploadFailedException(t("There was an unexpected error after uploading the file to THRON.")))->logException()->displayMessage();
        drupal_set_message(t("There was an unexpected error after uploading the file to THRON. Please contact your site administrator for more info."), 'warning');
      }
    }
  }
  ...
  public static function batchFinish($success, $results, $operations) {
    if (\Drupal::service('session')->get('upload_permissions', FALSE) != 'MEDIAUPLOADFORAPPROVAL') {
      ...
      \Drupal::service('session')->set('THRON_upload_batch_result', $results);
    }
  }
