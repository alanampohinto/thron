...
class Thronintegration_Api {
  ...
  private static $billingNamingConvention = "%s-billing.thron.com";

  private static $v4ConnectorNamingConvention = "%s-dev.thron.com";

  private static $cache = FALSE;

  private static $allowedAppTypes = [
    "REC_MASTER",
    "REC_SLAVE",
    "RTIE",
    "PLAY",
  ...
  /*
   * THRON integration: login as user (via identitymanager/login)
   */

  public static function performThronLogin($clientId, $username, $password) {
    try {
      $url = Thronintegration_Api::getThronEndpoint($clientId, "xsso") . "identitymanager/login";
      // perform login
      $tokenResp = Thronintegration_HTTP::doHTTP("POST", $url, "clientId=$clientId", [
        "X-USERNAME" => $username,
        "X-PASSWORD" => $password,
      ]);
  ...
  /*
   * THRON integration: get the user's details
   */

  public static function getUserDetails($clientId, $token, $username, $returnItags = FALSE, $offset = 0) {
    try {
      $url = Thronintegration_Api::getThronEndpoint($clientId, "xsso") . "vusermanager/detail?clientId=$clientId&username=$username&offset=$offset";
      if ($returnItags) {
        $url .= "&returnItags=true";
      }

      $detailResp = Thronintegration_HTTP::doHTTP("GET", $url, FALSE, ["X-TOKENID" => $token]);

      if (!$detailResp) {
        // error while logging in... TODO notify!
        throw new \Exception(sprintf("Could not get the users details for %s, username %s", $clientId, $username));
      }

      return json_decode($detailResp, FALSE);
    } catch (\Exception $ex) {
      // TODO notify the error!
      return FALSE;
    }
  ...
  /*
   * THRON integration: validate an user's token and return its deatils
   */

  public static function validateToken($clientId, $tokenId) {
    try {
      $url = Thronintegration_Api::getThronEndpoint($clientId, "xsso") . "accessmanager/validateToken/$clientId";
      $validateTokenResp = Thronintegration_HTTP::doHTTP("POST", $url, FALSE, ["X-TOKENID" => $tokenId]);
      //echo $url;
      //var_dump($validateTokenResp);exit;
      if (!$validateTokenResp) {
        // error while logging in... TODO notify!
        throw new \Exception("Could not validate token $tokenId on client $clientId!");
      }
      return json_decode($validateTokenResp, FALSE);
    } catch (\Exception $ex) {
      // TODO notify the error
      return FALSE;
    }
  ...
  public static function userFindByProperties($clientId, $token, $usernames = FALSE, $textSearch = FALSE, $linkedGroupsIds = FALSE, $returnRolesAndCapabilities = FALSE, $offset = 0, $numberOfResults = 50, $userRoles = FALSE) {
    try {
      $url = Thronintegration_Api::getThronEndpoint($clientId, "xsso") . "vusermanager/findByProperties";

      $findByPropertiesObj = new \stdClass();
      $findByPropertiesObj->clientId = $clientId;
      $findByPropertiesObj->criteria = new \stdClass();
      $findByPropertiesObj->orderBy = "firstName_A";
      $findByPropertiesObj->offset = $offset;
      $findByPropertiesObj->numberOfResult = $numberOfResults;

      if ($usernames && !Thronintegration_Utils::IsNullOrEmptyString($usernames)) {
        $findByPropertiesObj->criteria->usernames = explode(",", $usernames);
        //var_dump(json_encode($findByPropertiesObj));exit;
      }

      if ($textSearch && !Thronintegration_Utils::IsNullOrEmptyString($textSearch)) {
        $findByPropertiesObj->criteria->textSearch = $textSearch;
      }

      if ($linkedGroupsIds && !Thronintegration_Utils::IsNullOrEmptyString($linkedGroupsIds)) {
        $findByPropertiesObj->criteria->linkedGroupsIds = explode(",", $linkedGroupsIds);
      }

      if ($userRoles && !Thronintegration_Utils::IsNullOrEmptyString($userRoles)) {
        $findByPropertiesObj->criteria->userRoles = explode(",", $userRoles);
      }
      ...
      if (!$findByPropertiesResp) {
        throw new \Exception(sprintf("Cannot find users for client %s!", $clientId));
      }

      $obj = json_decode($findByPropertiesResp);
      if (!$obj) {
        throw new \Exception("Invalid response for client $clientId");
      }
      if (!$returnRolesAndCapabilities) {
        foreach ($obj->users as &$user) {
          unset($user->userDetail->userCapabilities);
        }
        unset($user);
      }
      return $obj;
  ...
  //this is just same as before but with changed criteria (by usertype)
  public static function userFindByPropertiesByType($clientId, $token, $usertypes, $offset, $numOfRes) {
    try {
      $url = Thronintegration_Api::getThronEndpoint($clientId, "xsso") . "vusermanager/findByProperties";

      $findByPropertiesObj = new \stdClass();
      $findByPropertiesObj->clientId = new \stdClass();
      $findByPropertiesObj->clientId = $clientId;
      $findByPropertiesObj->criteria = new \stdClass();
      $findByPropertiesObj->fieldOptions = new \stdClass();
      $findByPropertiesObj->fieldOptions->returnOwnAcl = TRUE;
      $findByPropertiesObj->fieldOptions->returnItags = TRUE;
      $findByPropertiesObj->fieldOptions->returnImetadata = TRUE;
      $findByPropertiesObj->orderBy = "firstName_A";
      $findByPropertiesObj->numberOfResult = $numOfRes;
      $findByPropertiesObj->offset = $offset;

      if (!Thronintegration_Utils::IsNullOrEmptyString($usertypes)) {
        $findByPropertiesObj->criteria->userType = explode(",", $usertypes);
      }
  ...
  //this is just same as before but with changed criteria (by groupId)
  public static function userFindByPropertiesByGroup($clientId, $token, $groupId, $offset, $numberOfResults = THRON_RESULTS_PER_PAGE) {
    try {
      $url = Thronintegration_Api::getThronEndpoint($clientId, "xsso") . "vusermanager/findByProperties";
      ...
      if (isset($offset) && intval($offset) > 0) {
        $findByPropertiesObj->offset = intval($offset);
      }

      if (isset($numberOfResults) && intval($numberOfResults) > 0) {
        $findByPropertiesObj->numberOfResult = $numberOfResults;
      }

      if (!Thronintegration_Utils::IsNullOrEmptyString($groupId)) {
        $findByPropertiesObj->criteria->linkedGroupsIds = explode(",", $groupId);
      }
      ...
      return json_decode($findByPropertiesResp);
    } catch (\Exception $ex) {
      // TODO notify!
      return FALSE;
    }
  ...
  /*
   * THRON integration: invokes the usersgroupmanager/findGroupsByProperties function
   */

  public static function findGroupByProperties($clientId, $token, $groupType, $textSearch, $returnRolesAndCapabilities = FALSE, $offset = 0, $numberOfResults = THRON_RESULTS_PER_PAGE) {
    try {
      $url = Thronintegration_Api::getThronEndpoint($clientId, "xsso") . "usersgroupmanager/findGroupsByProperties";

      $findByPropertiesObj = new \stdClass();
      $findByPropertiesObj->clientId = new \stdClass();
      $findByPropertiesObj->clientId = $clientId;
      $findByPropertiesObj->orderBy = "name_A";
      $findByPropertiesObj->criteria = new \stdClass();

      if (!Thronintegration_Utils::IsNullOrEmptyString($groupType)) {
        $findByPropertiesObj->criteria->groupType = explode(",", $groupType);
      }

      if (!Thronintegration_Utils::IsNullOrEmptyString($textSearch)) {
        $findByPropertiesObj->criteria->textSearch = $textSearch;
      }
      ...
      if (isset($numberOfResults) && intval($numberOfResults) > 0) {
        $findByPropertiesObj->numberOfResults = intval($numberOfResults);
      }

      $findByPropertiesResp = Thronintegration_HTTP::doHTTP("JSON_POST", $url, $findByPropertiesObj, ["X-TOKENID" => $token]);

      if (!$findByPropertiesResp) {
        throw new \Exception(sprintf("Cannot find groups for client %s!", $clientId));
      }
      ...
      if (!$returnRolesAndCapabilities) {
        foreach ($obj->groups as &$group) {
          unset($group->groupDetail->groupCapabilities);
          unset($group->groupDetail->imetadata);
          unset($group->groupDetail->itags);
        }
        unset($group);
      }
      return $obj;
  ...
  public static function findGroupByIds($clientId, $token, $groupIds, $offset = 0) {
    try {
      $url = Thronintegration_Api::getThronEndpoint($clientId, "xsso") . "usersgroupmanager/findGroupsByProperties";
      ...
      if (!Thronintegration_Utils::IsNullOrEmptyString($groupIds)) {
        $findByPropertiesObj->criteria->ids = explode(",", $groupIds);
      }
  ...
  public static function showContents($clientId, $token, $categoryId, $metadata = FALSE, $contentType = "IMAGE", $offset = 0, $numberOfResults = 50, $searchOnSubCategories = TRUE, $divArea = "720x0") {
    try {
      $url = Thronintegration_Api::getThronEndpoint($clientId, "xcontents") . "contentlist/showContents";

      $data = [
        "clientId" => $clientId,
        "onlyActiveContents" => TRUE,
        "contentType" => $contentType,
        "divArea" => $divArea,
        "searchOnSubCategories" => $searchOnSubCategories,
        "offset" => $offset,
        "numberOfResult" => $numberOfResults,
      ];

      if (isset($metadata) && !Thronintegration_Utils::IsNullOrEmptyString($metadata)) {
        $data["metadata"] = $metadata;
      }

      if (isset($categoryId) && !Thronintegration_Utils::IsNullOrEmptyString($categoryId)) {
      ...
      $contentDetailJson = Thronintegration_HTTP::doHTTP("GET", $url, $data, $headers);
      if (!$contentDetailJson) {
        throw new \Exception(sprintf("Unreadable content for %s", $clientId));
      }
  ...
  public static function showContentsWithIds($clientId, $token, $ids = [], $offset = 0, $numberOfResults = 50, $searchOnSubCategories = TRUE, $divArea = "720x0") {
    try {
      $url = Thronintegration_Api::getThronEndpoint($clientId, "xcontents") . "contentlist/showContents";
      ...
      /* echo "$url<br />";
        var_dump($data);
        var_dump($headers);
        var_dump(json_decode($contentDetailJson, false));
        exit(); */

      if (!$contentDetailJson) {
        throw new \Exception("Unreadable content for $clientId with " . count($ids) . " ids");
      }
  ...
  public static function getDownloadableContents($clientId, $token, $xcontentId, $pcontentId = FALSE, $divarea = FALSE) {
    try {
      $url = Thronintegration_Api::getThronEndpoint($clientId, "xcontents") . "delivery/getDownloadableContents";

      $data = ["clientId" => $clientId];
      if ($xcontentId) {
        $data["xcontentId"] = $xcontentId;
      }
      else {
        $data["xpublisherId"] = $pcontentId;
      }
      ...
      $contentsJson = Thronintegration_HTTP::doHTTP("GET", $url, $data, $headers);
      if (!$contentsJson) {
        throw new \Exception(sprintf("Unreadable content for %s, id %s", $clientId, $xcontentId));
      }

      return json_decode($contentsJson, FALSE);
    } catch (\Exception $ex) {
      return FALSE;
    }
  }

  public static function publishPagelet($clientId, $token, $owner, $postContent, $contentName, $defaultLocale, $categoryId) {
    try {
      $url = Thronintegration_Api::getThronEndpoint($clientId, "xadmin") . "publishinweeboexpress/publishPagelet";

      // publish this new content as a THRON pagelet
      $publishPageletObj = new \stdClass();
      $publishPageletObj->clientId = $clientId;
      $publishPageletObj->param = new \stdClass();
      $publishPageletObj->param->body = $postContent;
      $publishPageletObj->param->mimeType = "text/html";
      $publishPageletObj->param->initAllLocales = FALSE;
      $publishPageletObj->param->content = new \stdClass();
      $publishPageletObj->param->content->name = $contentName;
      $publishPageletObj->param->content->categoryId = $categoryId;
      $publishPageletObj->param->content->userId = $owner;
      $publishPageletObj->param->content->owner = $owner;
      $publishPageletObj->param->content->locale = $defaultLocale;
      $publishPageletObj->param->content->properties = [];

      $publishResp = Thronintegration_HTTP::doHTTP("JSON_POST", $url, $publishPageletObj, [
        "X-TOKENID" => $token,
        "Content-Type" => "application/json",
      ]);
      if (!$publishResp) {
        throw new \Exception(sprintf("Could not publish a new pagelet on %s: %s", $clientId, $publishResp));
      }

      return json_decode($publishResp, FALSE);
    } catch (\Exception $ex) {
      // TODO notify!
      return FALSE;
    }
  ...
  public static function updateContentLocale($clientId, $token, $pageletId, $name, $locale, $value) {
    try {
      // let's take care of the name and description for this locale
      $url = Thronintegration_Api::getThronEndpoint($clientId, "xcontents") . "content/updateContent4Locale";

      $updateLocaleObj = new \stdClass();
      $updateLocaleObj->client = new \stdClass();
      $updateLocaleObj->client->clientId = $clientId;
      $updateLocaleObj->contentId = $pageletId;
      $updateLocaleObj->detail = new \stdClass();
      $updateLocaleObj->detail->name = $name;
      $updateLocaleObj->detail->description = $value["description"];
      $updateLocaleObj->detail->locale = strtoupper($locale);

      $updateLocaleResp = Thronintegration_HTTP::doHTTP("JSON_POST", $url, $updateLocaleObj, ["X-TOKENID" => $token]);
      if ($updateLocaleResp) // TODO notify
      {
        ;
      }
  ...
  public static function getCategoriesForNames($clientId, $token, $lang, $ids) {
    try {
      ...
      while (count($ids) > 0) {
        $findCategoriesObj["properties"]["categoryIds"] = array_splice($ids, 0, 50);
        /*echo json_encode($findCategoriesObj);
        echo "\n";*/
        $findCategoriesResp = Thronintegration_HTTP::doHTTP("JSON_POST", $url, $findCategoriesObj, ["X-TOKENID" => $token]);

        $responseObj = json_decode($findCategoriesResp);

        foreach ($responseObj->categories as $categoryObj) {
          $category = $categoryObj->category;
          // locale
          $localeData = Thronintegration_Api::extractLocaleData($categoryObj->category->locales, $lang);
          $cat = ["id" => $category->id];
          if (count($localeData) > 0) {
            $localeElement = array_shift($localeData);
            if (!is_array($localeElement)) {
              $localeElement = (array) $localeElement;
            }

            if (!$localeElement || !isSet($localeElement["locale"])) {
              $cat["name"] = $localeElement["name"];
            }
            else {
              if ($localeElement["locale"] == $lang) {
                $cat["name"] = $localeElement["name"];
              }
              else {
                $cat["name"] = "[{$localeElement["locale"]}] {$localeElement["name"]}";
              }
            }
          }
          else {
      ...
      // TODO notify!
      return ["status" => "NO", "errorDescription" => $ex->getMessage()];
    }
  }

  public static function getCategories($clientId, $token, $lang, $rights = [], $ids = []) {
    try {
      $url = Thronintegration_Api::getThronEndpoint($clientId, "xcontents") . "category/findByProperties2";
      $categories = [];

      // if $rights is defined and there is at least one element let's pick the first right.
      $right = FALSE;
      if (is_array($rights) && count($rights) > 0) {
        $right = array_shift($rights);
      }

      $findCategoriesObj = new \stdClass();
      $findCategoriesObj->client = new \stdClass();
      $findCategoriesObj->client->clientId = $clientId;
      $findCategoriesObj->properties = new \stdClass();
      $findCategoriesObj->orderby = "categoryName_A";
      $findCategoriesObj->properties->excludeLevelHigherThan = 1;

      $findCategoriesObj->categoryFieldOption = new \stdClass();
      $findCategoriesObj->categoryFieldOption->returnNumberOfContents = FALSE;
      $findCategoriesObj->categoryFieldOption->returnNumberOfUnreadContents = FALSE;
      $findCategoriesObj->categoryFieldOption->returnNumberOfUnreadContentsInSubCategories = FALSE;
      $findCategoriesObj->categoryFieldOption->returnNumberOfSubCategories = FALSE;
      if ($right) {
        $findCategoriesObj->properties->acl = new \stdClass();
        $findCategoriesObj->properties->acl->rules = [$right];
      }
      if (count($ids) > 0) {
        $findCategoriesObj->properties->categoryIds = $ids;
      }

      $findCategoriesObj->numberOfResults = 50;

      $offset = 0;
      $ended = FALSE;
      while (!$ended):
        $findCategoriesObj->offset = $offset;
        $findCategoriesResp = Thronintegration_HTTP::doHTTP("JSON_POST", $url, $findCategoriesObj, ["X-TOKENID" => $token]);

        if (!$findCategoriesResp) {
          $ended = TRUE;
          break;
        }
        $responseObj = json_decode($findCategoriesResp);
        ...
        $activeCategories = array_filter($responseObj->categories, function ($obj) {
          return ($obj->category->active == TRUE);
        });

        if (is_array($rights) && count($rights) > 0) {
          $rightsArr = array_values($rights);
          while (count($rightsArr) > 0) {
            $categoryIds = array_map(function ($obj) {
              return $obj->category->id;
            }, $activeCategories);
            $subRight = array_shift($rightsArr);
            $findCategoriesSubObj = new \stdClass();
            $findCategoriesSubObj->client = new \stdClass();
            $findCategoriesSubObj->client->clientId = $clientId;
            $findCategoriesSubObj->properties = new \stdClass();
            $findCategoriesSubObj->properties->categoryIds = $categoryIds;
            $findCategoriesSubObj->orderby = "categoryName_A";
            $findCategoriesSubObj->properties->excludeLevelHigherThan = 1;
            $findCategoriesSubObj->properties->acl = new \stdClass();
            $findCategoriesSubObj->properties->acl->rules = [$subRight];
            $findCategoriesSubObj->numberOfResults = 50;
            $findCategoriesSubObj->offset = 0;
            $findCategoriesSubObj->categoryFieldOption = new \stdClass();
            $findCategoriesSubObj->categoryFieldOption->returnNumberOfContents = FALSE;
            $findCategoriesSubObj->categoryFieldOption->returnNumberOfUnreadContents = FALSE;
            $findCategoriesSubObj->categoryFieldOption->returnNumberOfUnreadContentsInSubCategories = FALSE;
            $findCategoriesSubObj->categoryFieldOption->returnNumberOfSubCategories = FALSE;

            $findCategoriesSubResp = Thronintegration_HTTP::doHTTP("JSON_POST", $url, $findCategoriesSubObj, ["X-TOKENID" => $token]);
            if (!$findCategoriesSubResp) {
              $activeCategories = [];
              $ended = TRUE;
              break;
            }
            $responseSubObj = json_decode($findCategoriesSubResp);
            if (!$responseSubObj) {
              $activeCategories = [];
              $ended = TRUE;
              break;
            }
            if (!isset($responseSubObj->categories) || count($responseSubObj->categories) == 0) {
              $activeCategories = [];
              $ended = TRUE;
              break;
            }

            $activeCategories = array_filter($responseSubObj->categories, function ($obj) {
              return ($obj->category->active == TRUE);
            });
          }
        }

        foreach ($activeCategories as $categoryObj) {
          $category = $categoryObj->category;
          // locale
          $localeData = Thronintegration_Api::extractLocaleData($categoryObj->category->locales, $lang);
          $cat = ["id" => $category->id];
        ...
        $offset += count($responseObj->categories);
  ...
  public static function categoryDetail($clientId, $token, $categoryId, $locale = FALSE) {
    $res = ["status" => "ERROR", "errorDescription" => ""];
    ...
    try {
      ...
      $url = Thronintegration_Api::getThronEndpoint($clientId, "xcontents") . "category/getCategory";
      $categoryDetailResp = Thronintegration_HTTP::doHTTP("GET", $url, $param, ["X-TOKENID" => $token]);

      if (!$categoryDetailResp) {
        // error while updating app... TODO notify!
        throw new \Exception(sprintf("Could not get the category's detail on %s, appId %s", $clientId, $categoryId));
      }

      $categoryObj = json_decode($categoryDetailResp, TRUE);
      if (!$categoryObj || !isset($categoryObj["resultCode"]) || $categoryObj["resultCode"] != "OK") {
  ...
  public static function extractLocaleData($dataObj, $lang, $nameField = "name", $localeField = "locale") {
    try {
      // ensure we're treating with a stdobj
      $data = json_decode(json_encode($dataObj), FALSE);

      $localeData = array_values(array_filter($data, function ($obj) use ($lang, $localeField) {
        return ($obj->$localeField == $lang);
      }));

      $localeFound = FALSE;
      if (count($localeData) > 0) {
        // is this locale empty?
        $localeElement = array_shift($localeData);
        if (isset($localeElement->$nameField) && !Thronintegration_Utils::IsNullOrEmptyString($localeElement->$nameField)) {
          $localeFound = TRUE;
        }
      }

      if (!$localeFound) {
        $localeData = array_values(array_filter($data, function ($obj) use ($localeField) {
          return ($obj->$localeField == "EN");
        }));

        if (count($localeData) > 0) {
          // is this locale empty?
          $localeElement = array_values($localeData);
          if (isset($localeElement->$nameField) && !Thronintegration_Utils::IsNullOrEmptyString($localeElement->$nameField)) {
            $localeFound = TRUE;
          }
        }
      ...
      if (!$localeFound) {
        // pick up the first locale
        $localeData = array_values(array_filter($data, function ($obj) use ($nameField) {
          return (isSet($obj->$nameField) && !Thronintegration_Utils::IsNullOrEmptyString($obj->$nameField));
        }));
      ...
      if ($localeFound) {
        if (is_array($localeElement)) {
          $localeData = (array) $localeElement;
        }
        else {
          $localeData = [];
          array_push($localeData, (array) $localeElement);
        }
      }
    } catch (\Exception $ex) {
      var_dump($ex);
    ...
    return json_decode(json_encode($localeData), TRUE);
  }
  ...
  public static function getUserRightsForApp($appDetails, $userName, $userCapabilities) {
    try {
      ...
      if ($appDetails["ownerUsername"] == $userName) {
        array_push($userRights, "APP_OWNER");
      }

      // is this user an administrator for this app? (==> 4ME_MANAGE_APPS)
      $searchManageAppsArr = array_filter($userCapabilities["userRoles"], function ($obj) {
        return $obj == "4ME_MANAGE_APPS";
      });
      if ($searchManageAppsArr && count($searchManageAppsArr) > 0) {
        array_push($userRights, "4ME_MANAGE_APPS");
      }

      // is this a manager for this app? check enabledSolutions and the user roles
      $appId = $appDetails["id"];
      $appInEnabledSolutionArr = array_filter($userCapabilities["enabledSolutions"], function ($obj) use ($appId) {
        return $obj == $appId;
      });
      if ($appInEnabledSolutionArr && count($appInEnabledSolutionArr) > 0) {
        // this app is present in the user's enabled solutions; check the user roles
        $userRolesForAppArr = array_filter($userCapabilities["userRoles"], function ($obj) use ($appId) {
          return Thronintegration_Utils::startsWith($obj, $appId);
        });

        if ($userRolesForAppArr && count($userRolesForAppArr) > 0) {
          foreach ($userRolesForAppArr as $userRole) {
            if (
              Thronintegration_Utils::endsWith($userRole, "_MANAGER") ||
              Thronintegration_Utils::endsWith($userRole, "ADMIN_USER")
            ) {
              array_push($userRights, "APP_MANAGER");
              break;
            }
            else {
              if (
              Thronintegration_Utils::endsWith($userRole, "APP_EDITOR")
              ) {
                array_push($userRights, "APP_EDITOR");
                break;
              }
            }
          }
  ...
  public static function getClientInformations($clientId) {
    try {
      ...
      $url = "http://$clientId-view.thron.com/ux/xbridge/resources/userutils/getClientInformations";
      // check the rules
      $clientInformationsResp = Thronintegration_HTTP::doHTTP("JSON_POST", $url, $param, ["Content-Type" => "application/json"]);
      if (!$clientInformationsResp) {
        // error while checking the rules
        throw new \Exception(sprintf("Could not get the client details on %s", $clientId));
      }
      return json_decode($clientInformationsResp, TRUE);
    } catch (\Exception $ex) {
      // TODO notify this error
      return FALSE;
    }
  ...
  private static function validateGetTHRONPlayerStyles($clientId, $tokenId) {
    if (!isset($clientId) || Thronintegration_Utils::IsNullOrEmptyString($clientId)) {
      throw new \Exception("invalid clientId");
    }
  ...
  public static function getTHRONPlayerStyles($clientId, $tokenId, $onlyActive = TRUE) {
    $res = ["status" => "ERROR", "errorDescription" => ""];

    try {
      Thronintegration_Api::validateGetTHRONPlayerStyles($clientId, $tokenId);

      // fetch these details from the cache, if present
      if (!Thronintegration_Api::$cache) {
        phpFastCache::setup("storage", "files");
        phpFastCache::setup("path", realpath(dirname(__FILE__)) . "/tmp/cache");
        Thronintegration_Api::$cache = phpFastCache();
      }

      $gfxCustomization = Thronintegration_Api::$cache->get("THRONPlayerStyles_$clientId");
      if (!$gfxCustomization) {
        $url = Thronintegration_Api::getThronEndpoint($clientId, "xcontents") . "client/detailClient?clientId=$clientId";
        $detailClientResp = Thronintegration_HTTP::doHTTP("GET", $url, FALSE, ["X-TOKENID" => $tokenId]);
        if (!$detailClientResp) {
          // error while creating app... TODO notify!
          throw new \Exception(sprintf("Could not get the client detail for %s", $clientId));
        }

        $detailClientRespObj = FALSE;
        try {
          $detailClientRespObj = json_decode($detailClientResp, TRUE);
        } catch (\Exception $e) {
          throw new \Exception(sprintf("Could not get the client detail for %s: invalid response from server", $clientId));
        }

        if (!$detailClientRespObj || !isset($detailClientRespObj["resultCode"]) || !isset($detailClientRespObj["resultCode"]) || $detailClientRespObj["resultCode"] != "OK") {
          throw new \Exception(sprintf("Could not get the client detail for %s: resultCode is not OK", $clientId));
        }

        $gfxCustomization = "";
        if (isset($detailClientRespObj["properties"]["embedTemplates"])) {
          if ($onlyActive) {
            $ret = array_filter($detailClientRespObj["properties"]["embedTemplates"], function ($obj) {
              return $obj["active"] == TRUE;
            });
          }
          else {
            $ret = $detailClientRespObj["properties"]["embedTemplates"];
          }


          $gfxCustomization = json_encode($ret);
        }

        Thronintegration_Api::$cache->set("THRONPlayerStyles_$clientId", $gfxCustomization, CACHE_DURATION);
      }
      $res["embedCodes"] = $gfxCustomization;
      $res["status"] = "OK";
    } catch (\Exception $ex) {
      // TODO notify this error
      $res["status"] = "ERROR";
  ...
  private static function validateClientPropertiesDetail($clientId, $token) {
    if (!isset($clientId) || Thronintegration_Utils::IsNullOrEmptyString($clientId)) {
      throw new \Exception("invalid clientId");
    }
  ...
  public static function clientPropertiesDetail($clientId, $token, $cache = TRUE) {
    try {
      Thronintegration_Api::validateClientPropertiesDetail($clientId, $token);
      ...
      // fetch these details from the cache, if present
      if ($cache) {
        $detailClientResp = Thronintegration_Api::$cache->get("clientPropertiesDetail_$clientId");
      }
      else {
        $detailClientResp = FALSE;
        try {
          Thronintegration_Api::$cache->delete("clientPropertiesDetail_$clientId");
        } catch (\Exception $e) {
        }
      }
      if (!$detailClientResp) {
        ...
        //$endPointCliendId = $clientConfig["clientId"];
        $url = Thronintegration_Api::getThronEndpoint($clientId, "xsso") . "clientproperties/detail";
        // check the rules
        $detailClientResp = Thronintegration_HTTP::doHTTP("GET", $url, $param, ["X-TOKENID" => $token]);
        //var_dump($detailClientResp);
        if (!$detailClientResp) {
          // error while checking the rules
          throw new \Exception(sprintf("Could not get the client details on %s", $clientId));
        }
        ...
        if ($cache) {
          Thronintegration_Api::$cache->set("clientPropertiesDetail_$clientId", $detailClientResp, CLIENT_PROPS_CACHE_DURATION);
        }
      }
      return json_decode($detailClientResp, TRUE);
    } catch (\Exception $ex) {
  ...
  private static function validateGetTrackingContextList($clientId, $tokenId) {
    if (!isset($clientId) || Thronintegration_Utils::IsNullOrEmptyString($clientId)) {
      throw new \Exception("invalid clientId");
    }
  ...
  public static function getTrackingContextList($clientId, $tokenId, $onlyActive = TRUE) {
    $res = ["status" => "ERROR", "errorDescription" => ""];

    try {
      Thronintegration_Api::validateGetTrackingContextList($clientId, $tokenId);
      ...
      $cacheKey = "THRONTrackingContexts_$clientId";
      if (!$onlyActive) {
        $cacheKey .= "_false";
      }

      $contexts = Thronintegration_Api::$cache->get($cacheKey);
      if (!$contexts) {
        $contexts = [];
        $url = Thronintegration_Api::getThronEndpoint($clientId, "xintelligence") . "context/list/$clientId";
        $params = new \stdClass();
        $params->limit = 50;
        $params->criteria = new \stdClass();
        ...
        while (!$ended) {
          ...
          $getContextResp = Thronintegration_HTTP::doHTTP("JSON_POST", $url, $params, ["X-TOKENID" => $tokenId]);

          if (!$getContextResp) {
            // error while getting the context list... TODO notify!
            throw new \Exception(sprintf("Could not get the tracking contexts for %s", $clientId));
          }

          $getContextRespObj = FALSE;
          try {
            $getContextRespObj = json_decode($getContextResp, TRUE);
          } catch (\Exception $e) {
            throw new \Exception(sprintf("Could not get the context list for %s: invalid response from server", $clientId));
          }

          if (!$getContextRespObj || !isset($getContextRespObj["resultCode"]) || !isset($getContextRespObj["resultCode"]) || $getContextRespObj["resultCode"] != "OK") {
            throw new \Exception(sprintf("Could not get the context list for %s: resultCode is not OK", $clientId));
          }


          if (count($getContextRespObj["items"]) == 0) {
            $ended = TRUE;
          }
          else {
            if ($onlyActive) {
              $contextList = array_filter($getContextRespObj["items"], function ($obj) {
                return $obj["removed"] == FALSE;
              });
            }
            else {
              $contextList = $getContextRespObj["items"];
            }

            foreach ($contextList as $context) {
              if (count($context["names"]) == 0) {
                $context["names"] = [
                  [
                    "lang" => "EN",
                    "label" => "({$context["id"]})",
            ...
            $offset += count($getContextRespObj["items"]);
          }
        }

        Thronintegration_Api::$cache->set($cacheKey, $contexts, GFX_CUSTOMIZATION_CACHE_DURATION);
      }
      $res["items"] = $contexts;
      $res["status"] = "OK";
    } catch (\Exception $ex) {
